...
- Ícono del ejecutable: se autogenera en la misma carpeta (Windows_Focus_icon.ico).

Compilar:
    pip install pyinstaller pystray pillow
    pyinstaller --onefile --noconsole --hidden-import=pystray._win32 --icon=Windows_Focus_icon.ico window_focus.py
...
# ──────────────────────────────────────────────
# Nombre fijo para archivo de ícono
APP_NAME = "Windows Focus"
ICON_FILENAME = "Windows_Focus_icon.ico"
...
# ──────────────────────────────────────────────
# Icono común (ventana + ejecutable)
# ──────────────────────────────────────────────
def create_app_icon():
    """Genera el icono y lo guarda en la carpeta del script/ejecutable si no existe.
    Retorna la ruta absoluta del archivo .ico."""
    # Obtener la ruta base (funciona tanto en script como en ejecutable PyInstaller)
    if getattr(sys, 'frozen', False):
        base_dir = os.path.dirname(sys.executable)
    else:
        base_dir = os.path.dirname(os.path.abspath(__file__))
    
    ico_path = os.path.join(base_dir, ICON_FILENAME)
    
    # Si ya existe, simplemente devuelve la ruta
    if os.path.exists(ico_path):
        return ico_path

    # Si no existe, lo generamos (primera ejecución)
    sz = 64
    ...
    try:
        ...
        # Si no se puede escribir (permisos), usar archivo temporal
        ico_path = os.path.join(tempfile.gettempdir(), ICON_FILENAME)
        img.save(ico_path, format="ICO", sizes=[(64, 64)])
        return ico_path
...
def show_selector():
    ...
    # Ícono de la ventana (el mismo que el del ejecutable)
    ico = create_app_icon()
...
def make_systray_image():
    """Crea la imagen que usará el systray (la misma que el .ico)"""
    sz = 64
    img = Image.new("RGBA", (sz, sz), (0, 0, 0, 0))
    d = ImageDraw.Draw(img)
    d.ellipse([0, 0, sz - 1, sz - 1], fill="#161622")
...
def apply_new_port(new_port):
    ...
    start_server(PORT)
    if tray_icon:
        tray_icon.title = f"{APP_NAME}  |  :{PORT}"
        menu = pystray.Menu(
